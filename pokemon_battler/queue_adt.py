...
class CircularQueue(Queue[T]):
    ...
    def __str__(self):
        """
        Reads through the queue and appends each str() method of each class to a return string 
        :pre:Requires the Queue to have items in it 
        :post: Returns a string of all method string methods separated by a commar
        :complexity: O(n) where n is the number of items in the array

        """
        
        rev_temp = CircularQueue(len(self))
        ret = CircularQueue(len(self))
        res = ""
        #look at each item 
        while(CircularQueue.is_empty(self) != True):
            string = self.serve()
            res += ","
            res += str(string)
            rev_temp.append(string)
        #return temp to original self
        #why does while(ArrayStack.is_empty(rev_temp) != True) not work
        while(CircularQueue.is_empty(rev_temp) != True):
        #while not rev_temp.is_empty():
            temp = rev_temp.serve()
            self.append(temp)
        #return original 
